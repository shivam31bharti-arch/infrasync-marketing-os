...
set -euo pipefail
# Git Bash on Windows rewrites arguments containing ':' (path conversion) and can
# mangle ffmpeg filter strings like subtitles=...:force_style='A=1,B=2' — disable it.
...
if [ -f captions.srt ]; then
  # NOTE: force_style values are ASS units (PlayResY=288), not pixels:
  # MarginV=40 ≈ 267px up from the bottom of a 1080x1920 frame (bottom safe zone).
  # FontSize=16 ≈ 107px tall text. Values ≥ 260 overflow and clamp to the TOP.
  ffmpeg -y -i "$SRC" -vf "subtitles=captions.srt:force_style='FontName=Arial,FontSize=16,Bold=1,Outline=2,MarginV=40,Alignment=2'" -c:a copy out/captioned.mp4; SRC=out/captioned.mp4
fi
