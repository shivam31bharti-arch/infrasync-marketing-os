...
## Setup (once)
1. Dev servers up: storefront (`npm run dev`, :3000) and Medusa (:9000). Start them if down.
...
3. Write `qa/snap.mjs`: Playwright script that captures FULL-PAGE screenshots of
   `/`, `/collections`, `/collections/clothing`, one product page, `/cart`, `/try-on`
...
2. **SEE + JUDGE** — open every screenshot and review AS IMAGES (you have vision; if your
   ...
   metrics only). Judge as a merciless senior art director at a luxury fashion house.
   ...
   Playwright hovers/scrolls) · mobile layout · cross-page consistency · "luxury feel"
   (would this embarrass Céline? then fix it) · accessibility (contrast, focus states,
   alt text) · performance (see #3). Write the scorecard + every concrete defect
   ("H1 clips at 390px on /collections", "card hover shadow too heavy — cheapens it")
...
3. **MEASURE** — `npx lighthouse http://localhost:3000 --preset=perf --chrome-flags="--headless" --output=json --output-path=qa/shots/iter-N/lh.json`
   (and the same for one product page). Record mobile perf + a11y scores and total JS KB.
...
- Playwright walkthrough: add product to cart → cart updates · subscribe form happy + error
  path · try-on with the DEMO MODEL path completes and renders a composite (never test with a
  real person's photo). Any break = fix before visual polish continues.
...
no user-photo storage · keep Medusa/Supabase/analytics wiring working (walkthrough proves it) ·
